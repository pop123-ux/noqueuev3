/**
 * Onboarding — Activează Seiful de Identitate
 * Shown after first login to explain the Identity Vault and collect granular consents.
 */
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';
import { Shield, CheckCircle2, ArrowRight, Loader2, Lock, FileText, Cloud, Zap } from 'lucide-react';
...
export default function Onboarding() {
  ...
  const [step, setStep] = useState(0); // 0=intro, 1=consents, 2=done
  ...
  const handleActivate = async () => {
    ...
    if (!user?.email) { navigate('/profile'); return; }
    ...
    // Ensure IdentitySecret shell exists
    ...
    // Mark onboarding complete on UserPrivateProfile
    ...
    <div className="min-h-screen bg-background text-foreground flex items-center justify-center px-4">
      <div className="w-full max-w-lg">
        <AnimatePresence mode="wait">
          {step === 0 && (
            <motion.div key="intro" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              ...
              <div className="text-center mb-8">
                ...
                <h1 className="text-2xl font-bold text-white mb-3">Activează Seiful de Identitate</h1>
              ...
              <Button onClick={() => setStep(1)} className="w-full h-11 rounded-xl bg-primary">
                Continuă <ArrowRight className="w-4 h-4 ml-2" />
              </Button>
              <button onClick={() => navigate('/')} className="w-full mt-3 text-xs text-slate-500 hover:text-slate-400 transition-colors">
                Fă asta mai târziu
              </button>
            </motion.div>
          )}
          ...
          {step === 1 && (
            <motion.div key="consents" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
                ...
                {CONSENTS.map(c => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left glass-card rounded-2xl p-4 border transition-all ${
                        checked ? 'border-primary/40 bg-primary/5' : 'border-white/8'
                      }`}
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <p className="text-sm text-white leading-snug">
                            {c.label}
                            {c.required && <span className="text-primary ml-1">*</span>}
                          </p>
              ...
              <Button
                ...
                className="w-full h-11 rounded-xl bg-primary disabled:opacity-40"
              >
                {saving ? (
                  <><Loader2 className="w-4 h-4 animate-spin mr-2" />Se activează...</>
                ) : (
                  <><Shield className="w-4 h-4 mr-2" />Activează Seiful de Identitate</>
                )}
              </Button>
              <button onClick={() => setStep(0)} className="w-full mt-3 text-xs text-slate-500 hover:text-slate-400 transition-colors">
                Înapoi
              </button>
            </motion.div>
          )}
          ...
          {step === 2 && (
            <motion.div key="done" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="text-center">
              ...
              <p className="text-slate-400 text-sm mb-8">
                Acum poți completa datele tale și le vom folosi la auto-completarea documentelor.
              </p>
              <Button onClick={() => navigate('/profile')} className="w-full h-11 rounded-xl bg-primary">
                <Zap className="w-4 h-4 mr-2" />
                Completează Seiful de Identitate
              </Button>
            </motion.div>
          )}
        </AnimatePresence>
